...
from uncertainty.predictive import has_active_dropout, predict_distribution
...
def parse_args() -> argparse.Namespace:
    ...
    # True labels build clean regime distributions; online use can mix them softly.
    parser.add_argument("--group_source", choices=["predicted", "true", "none"], default="true")
...
def main() -> None:
    ...
    model.eval()
    if args.mc_samples > 1 and not has_active_dropout(model):
        print("[warning] mc_samples>1 but the checkpoint has no active dropout; uncertainty will be zero")
    ...
    for x, state_history, _, u_last, regime, params, _ in loader:
        ...
        distribution = predict_distribution(
            model, x, state_history, params_device, mc_samples=args.mc_samples,
        )
        prediction, logits = distribution.mean, distribution.logits
        coeff_ood_np = ood_scorer.score_mapping(params)
        coeff_ood = torch.as_tensor(coeff_ood_np, device=device, dtype=prediction.dtype)
        components = risk_components_torch(
            prediction, u_last, x, params_device,
            coeff_ood=coeff_ood,
