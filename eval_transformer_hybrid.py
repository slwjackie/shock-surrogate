...
@torch.no_grad()
def evaluate_split(model, loader, device: torch.device) -> dict[str, float]:
    squared_error = 0.0
    absolute_error = 0.0
    element_count = 0
    correct = 0
...
def main() -> None:
    ...
    checkpoint_path = Path(args.checkpoint) if args.checkpoint else Path(args.ckpt_dir) / f"best_{guessed_arch}_{args.mode}_seed{args.seed}_H{args.H}.pt"
    ...
    for name, split, path in splits:
        ...
        results[name] = evaluate_split(model, loader, device)

    if args.save_metrics:
        output = Path(args.out_metrics) if args.out_metrics else Path("outputs") / f"metrics_{arch}_{args.mode}_seed{args.seed}_H{history}.json"
